...
const Feeds: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Headlines List - OPTIMIZED with Virtual Scrolling */}
      {filteredHeadlines.length === 0 ? (
        ...
        <div>
          <List
            height={800}
            itemCount={filteredHeadlines.length}
            itemSize={200}
            width="100%"
            itemData={filteredHeadlines}
          >
            {({ index, style, data }) => {
              const headline = data[index]
              return (
                <div style={style} className="px-1 pb-4">
                  <div className="card hover:bg-white/10 transition-all duration-300 group h-full">
                    <div className="flex items-start justify-between gap-4 h-full">
                      <div className="flex-1 min-w-0">
                        ...
                        {headline.original_title && (
                          <div className="bg-white/5 p-2 rounded-lg border border-white/10 mb-2 backdrop-blur-sm">
                            <div className="text-xs text-gray-400 mb-1">Original:</div>
                            <div className="text-sm text-gray-200 line-clamp-1">{headline.original_title}</div>
                          </div>
                        )}
                        ...
                        <div className="flex items-center flex-wrap gap-3 text-sm text-gray-400">
                          <span className="flex items-center space-x-1">
                            ...
                            <span className="truncate max-w-[150px]">{headline.source}</span>
                          </span>
                          <span className="flex items-center space-x-1">
                          ...
                          {headline.published && (
                            <span className="flex items-center space-x-1">
                              ...
                              <span className="text-xs truncate">{formatDate((headline as any).published)}</span>
                            </span>
                          )}
                          {headline.translated && (
                            <span className="glass-badge bg-gradient-to-r from-green-500/20 to-emerald-500/20 text-green-400 border-green-400/30 text-xs">
                              Translated
                            </span>
                          )}
                        </div>
